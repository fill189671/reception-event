...
async def on_message(message): # メッセージが送られたら
	...
	for h,i in enumerate(table_num) : # 卓数だけ繰り返す
		...
		table_info_env = os.environ['TABLE_'+ str(i)] # 各卓情報（環境変数に格納） info_a,info_b,info_c...の形で変数宣言しとく。中身は"name":"A卓：【卓名】","gm":"GM名","uw_ch":打ち合わせチャンネルID,"za_ch":雑談チャンネルID,"mem":"twitter_1"/"twitter_2"
		...
		mem_list = table[h]["mem"].split("/") # 参加者のTwitterアカウントをリスト化する
		table[h]["mem"] = mem_list # 元の辞書の値にリスト型で戻す
